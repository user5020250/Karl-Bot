...
SETTINGS_KEY = "automod2"  # distinct storage key, keeps old data untouched
...
# ============================================================
# MODALS (numeric configuration)
# ============================================================

class _BaseConfigModal(discord.ui.Modal):
    """Shared submit logic for all configurable-filter modals."""

    filter_key: str = ""
    fields: list = []  # list of (attr_name, TextInput label, default_getter)
    ...
    async def _save(self, interaction: discord.Interaction, parsed: dict):
        settings = get_settings(self.guild_id)
        cfg = settings.get(self.filter_key, dict(DEFAULTS[self.filter_key]))
        cfg.update(parsed)
        update_setting(self.guild_id, self.filter_key, cfg)
        await self.cog.refresh_panel(interaction)


class SpamModal(_BaseConfigModal, title="Configure Anti Spam"):
    filter_key = "spam"

    def __init__(self, cog, guild_id, current):
        super().__init__(cog, guild_id, current)
        self.limit = discord.ui.TextInput(
            label="Message limit", default=str(current.get("limit", 5))
        )
        self.seconds = discord.ui.TextInput(
            label="Time window (seconds)", default=str(current.get("seconds", 5))
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            parsed = {"limit": int(self.limit.value), "seconds": int(self.seconds.value)}
        except ValueError:
            await interaction.response.send_message(
                "Limit and seconds must be numbers.", ephemeral=True
            )
            return
        await self._save(interaction, parsed)


class MentionsModal(_BaseConfigModal, title="Configure Mention Limit"):
    filter_key = "mentions"

    def __init__(self, cog, guild_id, current):
        super().__init__(cog, guild_id, current)
        self.limit = discord.ui.TextInput(
            label="Mention limit", default=str(current.get("limit", 5))
        )
        self.add_item(self.limit)

    async def on_submit(self, interaction: discord.Interaction):
        try:
            parsed = {"limit": int(self.limit.value)}
        except ValueError:
            await interaction.response.send_message(
                "Limit must be a number.", ephemeral=True
            )
            return
        await self._save(interaction, parsed)


class RaidModal(_BaseConfigModal, title="Configure Raid Protection"):
    filter_key = "raid"

    def __init__(self, cog, guild_id, current):
        super().__init__(cog, guild_id, current)
        self.joins = discord.ui.TextInput(
            label="Join count", default=str(current.get("joins", 10))
        )
        self.seconds = discord.ui.TextInput(
            label="Time window (seconds)", default=str(current.get("seconds", 30))
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            parsed = {"joins": int(self.joins.value), "seconds": int(self.seconds.value)}
        except ValueError:
            await interaction.response.send_message(
                "Joins and seconds must be numbers.", ephemeral=True
            )
            return
        await self._save(interaction, parsed)


class EmojiModal(_BaseConfigModal, title="Configure Emoji Limit"):
    filter_key = "emoji"

    def __init__(self, cog, guild_id, current):
        super().__init__(cog, guild_id, current)
        self.limit = discord.ui.TextInput(
            label="Emoji limit", default=str(current.get("limit", 10))
        )
        self.add_item(self.limit)
...
class CapsModal(_BaseConfigModal, title="Configure Caps Filter"):
    filter_key = "caps"

    def __init__(self, cog, guild_id, current):
        super().__init__(cog, guild_id, current)
        self.percent = discord.ui.TextInput(
            label="Max caps percentage", default=str(current.get("percent", 70))
        )
        self.add_item(self.percent)

    async def on_submit(self, interaction: discord.Interaction):
        try:
            parsed = {"percent": int(self.percent.value)}
        except ValueError:
            await interaction.response.send_message(
                "Percentage must be a number.", ephemeral=True
            )
            return
        await self._save(interaction, parsed)
...
class ConfigureSelect(discord.ui.Select):
    def __init__(self, cog, guild_id):
        options = [
            discord.SelectOption(label=f"Configure {label}", value=key)
...
# COMMAND GROUPS
# ============================================================
# Renamed top-level group to "shield" to avoid colliding with any
# leftover "automod" command registration from a previous version.
...
class AutoMod(commands.Cog):
    ...
    async def shield_panel(self, interaction: discord.Interaction):
        embed = build_panel_embed(interaction.guild.id)
        view = AutoModPanelView(self, interaction.guild.id)
    ...
    async def profanity_list(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=make_embed(
                "Blocked Words", ", ".join(words) if words else "No blocked words"
            ),
            ephemeral=True,
        )
